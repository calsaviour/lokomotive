...
spec:
  ...
  - image: fedora
    ...
    args:
    - -c
    - 'curl -sLO https://storage.googleapis.com/kubernetes-release/release/$(curl -s https://storage.googleapis.com/kubernetes-release/release/stable.txt)/bin/linux/amd64/kubectl && chmod +x ./kubectl &&  mv ./kubectl /usr/local/bin/ && kubectl version -o yaml && curl -sL https://run.linkerd.io/install | sh && export PATH=$PATH:$HOME/.linkerd2/bin && linkerd version && linkerd check'
    volumeMounts:
    - name: kubeconfig
      mountPath: /root/.kube
  volumes:
...
func TestLinkerdCheck(t *testing.T) {
	...
	_, err = client.ConfigMaps(ns.Name).Create(context.TODO(), getKubeConfigMap(t), metav1.CreateOptions{})
	if err != nil {
...
func getKubeConfigMap(t *testing.T) *corev1.ConfigMap {
